...
# 确认所有任务已完成
grep -c "\- \[x\]" changes/<change-id>/tasks.md
```
...
3. 运行本功能测试：pytest tests/xxx_test.py -v
   【注意】这只是单功能测试，全量测试在步骤 6 执行
4. 测试全绿 → 打勾，进入下一个任务
   测试失败 → 修复代码/测试 → 重测（最多 3 次）
```

**重要区分**：
- **步骤 5（本步骤）**：每个任务完成后只运行本功能测试，用于验证功能正确性
- **步骤 6（归档前）**：运行全量测试（所有 pytest + 所有 e2e），确保整体系统无回归
